      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8">
          ...
          {testimonials.map((t) => (
            ...
            >
              ...
              <div className="flex gap-1">
                {[...Array(5)].map((_, i) => (
                  <StarIcon key={i} className="size-4 fill-[#378ADD] text-[#378ADD]" />
                ))}
              </div>
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                <div className="w-11 h-11 rounded-full bg-[#185FA5] flex items-center justify-center text-[15px] font-medium text-[#85b7eb] flex-shrink-0">
                  {t.avatar}
                </div>
                <div>
                  <p className="text-[15px] font-medium text-slate-800">{t.name}</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 rounded-2xl border border-slate-200 bg-white overflow-hidden">
          {stats.map((s, i) => (
            ...
            >
              <p className="text-[26px] font-medium text-slate-900">{s.val}</p>
              <p className="text-[13px] text-slate-400 mt-1">{s.label}</p>
            </div>
          ))}
        </div>
